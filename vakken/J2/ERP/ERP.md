...
Business process = Een serie acties die, wanneer ze in de juiste volgorde worden uitgevoerd (door machines en/of mensen), een service of product als resultaat krijgen. Bedrijfsprocessen komen ook binnen het bedrijf zelf voor (resultaat is niet voor de klant maar voor het bedrijf zelf).
...
Introduction to SAP (gwn ne keer goe lezen, vetgedrukt = belangrijk)
Flexso: Bedrijf dat customization, consultancy en implementations gaat doen bij klanten.
SAP: Systems, Applications and Products in data processing
ERP software geschreven in programmeertaal ABAP
Founded in 1972 in Germany, by 5 former IBM workers. Started from the bottom met geleende computers en maakten na het eerste jaar meer dan een half miljoen winst (ez clap zegt)
...
Veel meer cijfermateriaal (-> SAP Leonardo)
Veel meer nood aan programma’s om dat te kunnen verwerken (-> SAP Digital Core)
[Digital Core](https://news.sap.com/2016/06/sap-s4hana-the-digital-core/): de SAP HANA Database (werking)
...
```
Integraties met bijvoorbeeld SAP Successfactors (HR programma)
...
```
[SAP Leonardo](https://www.sap.com/products/leonardo.html): Machine learning
...
-> gewoon een beeke weten waar de titeltjes voor staan: Machine learning, Internet of Things (IOT), Analytics, Blockchain, Data Intelligence, Big Data. Zij zijn onderdeel van SAP Leonardo. Cloud platform: alles draait in de cloud! (soms mogelijk on premise)

De Digital Core is een SAP database waar programma’s die SAP aanbiedt op draaien, waar je de data in verwerkt die je mogelijks verkrijgt via SAP Leonardo.
...
Kort: 
MRP: resources = SAP R/2
MRP II: MRP + planning + cost analyse + PLM = SAP R/3
ERP: MRP + MRP II + verkoop + aankoop + financiën + human resources = SAP ERP
(SAP HANA: in memory database)
Suite on HANA: ERP met deftige UI: Fiori UX
SAP S/4HANA: Digital core: opvolger van ERP draaiende op HANA database
