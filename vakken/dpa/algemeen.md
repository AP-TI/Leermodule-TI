# DPA
## Het grote "ik doe niks verkeerd maar da werkt hier ni"-kennissysteem
### Deel 4
- [Waarom krijg ik gene mail?](fixmijnemail.md)
- [Die testmail werkte MAAR NU KRIJG IK GEEN MAILS VAN MIJN ALERTS](fixmijnemailtwee.md)
- [Mijn CPU-Alert werkte perfect, maar de alert voor error 102 werkt niet](fixdaalert.md)
### Deel 5
- [IK VIND SQL Server Data Tools 2015 NIET HELPPPPPPPPP](ssdt.md)
- [SQL Server Native Client 11.0 is niet zichtbaar bij Choose a Destination in de SQL Server Import and Export Wizard](les5sqlservernativeclient.md)


Niet gevonden wat je zocht? Stuur me gerust op discord @mixxamm.
